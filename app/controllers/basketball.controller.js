const BasketBall = require("../models/baskteball.model");

exports.addMatch = async (req, res) => {
  try {
    const body = req.body;
    console.log("basketball: ", body);

    const newCric = new BasketBall(body);
    await newCric.save();
...
exports.updateMatch = async (req, res) => {
  try {
    ...
    const updateMatch = await BasketBall.findByIdAndUpdate(matchId, req.body, {
      new: true,
    });
...
exports.matchDelete = async (req, res) => {
  try {
    ...
    const deleteMatch = await BasketBall.findByIdAndDelete(matchId);
...
exports.getLiveMatch = async (req, res) => {
  try {
    const matches = await BasketBall.find({ matchStatus: "live" });
...
exports.getCompletdMatch = async (req, res) => {
  try {
    const matches = await BasketBall.find({ matchStatus: "completed" });
...
exports. getScheduledMatch = async (req, res) => {
  try {
    const matches = await BasketBall.find({ matchStatus: "scheduled" });
