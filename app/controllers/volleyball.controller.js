const VolleyBall = require("../models/volleyball.model");

exports.addMatch = async (req, res) => {
  try {
    const body = req.body;

    const newCric = new VolleyBall(body);
    await newCric.save();
...
exports.updateMatch = async (req, res) => {
  try {
    ...
    const updateMatch = await VolleyBall.findByIdAndUpdate(matchId, req.body, {
      new: true,
    });
...
exports.matchDelete = async (req, res) => {
  try {
    ...
    const deleteMatch = await VolleyBall.findByIdAndDelete(matchId);
...
exports.getLiveMatch = async (req, res) => {
  try {
    const matches = await VolleyBall.find({ matchStatus: "live" });
...
exports.getCompletdMatch = async (req, res) => {
  try {
    const matches = await VolleyBall.find({ matchStatus: "completed" });
...
exports.getScheduledMatch = async (req, res) => {
  try {
    const matches = await VolleyBall.find({ matchStatus: "scheduled" });
